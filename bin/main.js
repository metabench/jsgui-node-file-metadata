...
var file_metadata = require('jsgui-node-file-metadata');
...
var program = require('commander');
  ...
  .option('-p, --file_path [value]', 'Input file path')
  .parse(process.argv);
...
if (process.argv.length > 2) {
    ...
    if (cl_args_no_dash.length == 1) {
        var file_path = cl_args_no_dash[0];
        
        
        file_metadata.from_file(file_path, function(err, res_metadata) {
            if (err) {
        ...
        var files_paths = cl_args_no_dash;
        file_metadata.from_file(files_paths, function(err, res_metadata) {
            if (err) {
